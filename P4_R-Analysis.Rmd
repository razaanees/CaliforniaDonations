...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
p1 <- ggplot(aes(x=contb_receipt_amt), data=subset(donations, contb_receipt_amt > 0 & !is.na(contb_receipt_amt))) +
  geom_histogram() +
...
democrats <- c('Sanders, Bernard', "Clinton, Hillary Rodham", 'Lessig, Lawrence', 'Webb, James Henry Jr.')
...
donations$party <- factor(donations$party, levels=c("Democratic", "Libertarian", "Green", "Republican", ordered=TRUE))
```
...
ggplot(as.data.frame(sort(table(donations$contbr_city), decreasing=T)[1:10]), aes(x=Var1, y=Freq)) +
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
ggplot(as.data.frame(sort(table(donations$contbr_employer), decreasing=T)[1:10]), aes(x=Var1, y=Freq)) +
  geom_bar(stat='identity') +
  theme(axis.text.x=element_text(angle=45, hjust=1)) +
  xlab('Employer') +
  ylab('Count')
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
ggplot(as.data.frame(sort(table(donations$contbr_occupation), decreasing=T)[1:10]), aes(x=Var1, y=Freq)) +
  geom_bar(stat='identity') +
  theme(axis.text.x=element_text(angle=45, hjust=1)) +
  xlab('Employer') +
  ylab('Count')
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
ggplot(data=subset(donations, contbr_occupation== "RETIRED" | contbr_occupation == "SELF-EMPLOYED"), aes(x=cand_nm)) +
  geom_bar(stat="count") +
  theme(axis.text.x=element_text(angle=30, hjust=1))+
...
The boxplot of number of donations for each candidate had large outliers so a second plot shows the 95% quantile of the data as the upper limit on the y-axis.
...
```{r, echo=FALSE, error=FALSE, warning=FALSE}
sum.donations.occ <- by(donations$contb_receipt_amt, donations$contbr_occupation, sum)
...
```{r, echo=FALSE, error=FALSE, warning=FALSE}
sum.donations.emp <- by(donations$contb_receipt_amt, donations$contbr_employer, sum)
...
cities=c("LOS ANGELES", "SAN FRANCISCO", "SAN DIEGO", "OAKLAND", "SAN JOSE", "SACRAMENTO", "BERKELEY", "LONG BEACH", "SANTA MONICA", "SANTA BARBARA")
cands=c("Clinton, Hillary Rodham", "Trump, Donald J.", "Johnson, Gary", "Stein, Jill")
...
occupations=c("RETIRED", "NOT EMPLOYED", "ATTORNEY", "TEACHER", "ENGINEER", "HOMEMAKER", "SOFTWARE ENGINEER", "PHYSICIAN", "PROFESSOR", "CEO")
cands=c("Clinton, Hillary Rodham", "Trump, Donald J.", "Johnson, Gary", "Stein, Jill")
...
ggplot(data=subset(donations, contb_receipt_amt >0), aes(x=cand_nm, y=contb_receipt_amt)) +
...
### Description Two
This plot shows the difference in the value of single donations that each candidate received. Some candidates received a wide range of donations and have large inter-quartile ranges. There are also a lot of outliers for each candidate.

From the graph, Bernie Sanders has the smallest IQR but also a lot of outliers. The largest IQR are for Republican candidates Jeb Bush and Chris Christie. Neither of them received a large number of donations so accurate statistics are difficult to determine in their cases.
...
ggplot(data=donations_party, aes(x=city, y=sum)) +
  geom_bar(stat='identity', aes(fill=party)) +
  scale_fill_brewer(palette ='Set3', guide=guide_legend(title='Party')) +
  scale_y_continuous(labels=comma, breaks=seq(0,15000000, 2000000)) +
  theme(axis.text.x=element_text(angle=30, hjust=1)) +
  ylab('Total Amount Donated ($)') +
  xlab("City")
```
...
### Description Three
The final plots I want to highlight show the sum of donations from the top 10 contributing cities to the candidates and political parties. These plots show several things:

* Hillary Clinton and the Democratic Party are overwhelmingly supported in each of the top 10 cities.

* Almost no support for the Libertarian or Green Party or their nominees.

* Less support for the Republican nominee, Donald Trump, then for the overall Republican Party.
